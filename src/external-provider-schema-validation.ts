...
export async function validateExternalProviderCatalogSchema(input: { readonly architectureRoot: string; readonly value: ExternalProvidersCatalog; }): Promise<readonly Diagnostic[]> {
  const validate = await compileExternalProviderSchema(input.architectureRoot);
  const valid = validate(input.value);
  const errors = validate.errors ?? [];
  return valid ? [] : [{ ruleId: EXTERNAL_PROVIDER_SCHEMA_RULE_ID, severity: 'error', file: EXTERNAL_PROVIDER_CATALOG_FILE,
    path: toDiagnosticPath(errors[0]), message: `External provider catalog violates \`${EXTERNAL_PROVIDER_SCHEMA_FILE}\`: ${formatSchemaErrors(errors)}` }];
}
async function compileExternalProviderSchema(architectureRoot: string): Promise<ValidateFunction> {
  return compileJsonSchemaFile({ absolutePath: join(architectureRoot, EXTERNAL_PROVIDER_SCHEMA_FILE), allowedRoot: architectureRoot });
}
