...
export async function loadArchitectureCatalogSchemaPreflight(architectureRoot: string): Promise<ArchitectureCatalogSchemaPreflight> {
  const catalogs = await loadArchitectureCatalogs(architectureRoot);
  const validation = await validateArchitectureCatalogSchemas({ architectureRoot, catalogs });
...
export async function validateArchitectureCatalogSchemas(input: {
  ...
  const diagnostics = (await Promise.all([
    ...
    validateOperationalAssetCatalogSchema({ architectureRoot: input.architectureRoot, value: input.catalogs.operationalAssets, observedAt: input.observedAt }),
