...
export async function validateDataClassCatalogSchema(input: { readonly architectureRoot: string; readonly value: DataClassesCatalog; }): Promise<readonly Diagnostic[]> {
  const validate = await compileDataClassSchema(input.architectureRoot);
  const valid = validate(input.value);
  const errors = validate.errors ?? [];
  return valid ? [] : [{ ruleId: DATA_CLASS_SCHEMA_RULE_ID, severity: 'error', file: DATA_CLASS_CATALOG_FILE,
    path: toDiagnosticPath(errors[0]), message: `Data class catalog violates \`${DATA_CLASS_SCHEMA_FILE}\`: ${formatSchemaErrors(errors)}` }];
}
async function compileDataClassSchema(architectureRoot: string): Promise<ValidateFunction> {
  return compileJsonSchemaFile({ absolutePath: join(architectureRoot, DATA_CLASS_SCHEMA_FILE), allowedRoot: architectureRoot });
}
