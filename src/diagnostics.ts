...
/** Legacy identity remains available for stored v1 reports. */
...
/** IDs identify records; array offsets only identify records without IDs. */
export function getStableDiagnosticFingerprint(diagnostic: Diagnostic): string {
  const path = diagnostic.path.trim().replace(
    /\[(?:\d+:|id=)([^\]]+)\]/g,
    (_match: string, id: string) => `[@${JSON.stringify(id)}]`
  );
...
function fingerprint(diagnostic: Diagnostic, namespace: string, path: string): string {
  ...
  return createHash('sha256').update([
    namespace, diagnostic.ruleId.trim(), normalizeDiagnosticFile(diagnostic.file), path
  ].join('\0')).digest('hex');
}
