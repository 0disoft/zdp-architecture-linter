import { getArchitectureCatalogSourceRoot } from './architecture-source-root.ts';
...
export function createArchitectureDiffReport(input: CreateArchitectureDiffReportInput): ArchitectureDiffReport {
  ...
  const compatibilityDiagnostics = createEventSchemaCompatibilityDiagnostics(input.baseCatalogs, input.headCatalogs);
  ...
  const diagnostics = diffDiagnostics(input.baseDiagnostics, [...input.headDiagnostics, ...compatibilityDiagnostics]);
...
function createEventSchemaCompatibilityDiagnostics(baseCatalogs: ArchitectureCatalogs, headCatalogs: ArchitectureCatalogs): readonly Diagnostic[] {
  ...
  if (baseArchitectureRoot === undefined || headArchitectureRoot === undefined) return [];
  return validateEventSchemaCompatibility({ baseArchitectureRoot, headArchitectureRoot });
}
...
function createRepositoryRiskNotes(baseCatalogs: ArchitectureCatalogs, headCatalogs: ArchitectureCatalogs, diff: CatalogCollectionDiff): readonly string[] {
  const baseById = mapById(getCollection(baseCatalogs.repositories.repositories, 'name'));
  const headById = mapById(getCollection(headCatalogs.repositories.repositories, 'name'));
  return diff.changed.flatMap((id) => createFieldChangeNotes({ collection: 'repositories', id, baseValue: baseById.get(id), headValue: headById.get(id), fields: ['repo_stage', 'kind', 'owner', 'risk_level', 'area', 'agent_review'] }));
}
function createServiceRiskNotes(baseCatalogs: ArchitectureCatalogs, headCatalogs: ArchitectureCatalogs, diff: CatalogCollectionDiff): readonly string[] {
  ...
  return diff.changed.flatMap((id) => createFieldChangeNotes({ collection: 'services', id, baseValue: baseById.get(id), headValue: headById.get(id), fields: [
...
function createOperationalAssetRiskNotes(baseCatalogs: ArchitectureCatalogs, headCatalogs: ArchitectureCatalogs, diff: CatalogCollectionDiff): readonly string[] {
  const baseById = mapById(getCollection(baseCatalogs.operationalAssets?.assets, 'id'));
  const headById = mapById(getCollection(headCatalogs.operationalAssets?.assets, 'id'));
  return diff.changed.flatMap((id) => createFieldChangeNotes({ collection: 'operationalAssets', id, baseValue: baseById.get(id), headValue: headById.get(id), fields: ['status', 'environment', 'criticality', 'security.public_access'] }));
}
...
function formatCollection(name: CollectionName, diff: CatalogCollectionDiff): readonly string[] {
  return [`## ${name}`, `- added: ${formatInlineList(diff.added)}`, `- removed: ${formatInlineList(diff.removed)}`, `- changed: ${formatInlineList(diff.changed)}`];
}
