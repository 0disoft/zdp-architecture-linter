...
저장소 부트스트랩, repository catalog 필수 필드 검사, service repo 참조 검사, 서비스 의존성 참조 검사, 데이터 저장소 참조 검사, 외부 제공자 참조 검사, 엣지 런타임의 상태 저장소 직접 접근 차단은 구현됐다. 현재 `zdp-architecture`의 실제 카탈로그는 `ZDP-REPO-001`, `ZDP-REPO-002`, `ZDP-REF-001`, `ZDP-REF-002`, `ZDP-REF-003`, `ZDP-REF-004`, `ZDP-REF-005`, `ZDP-DATA-004` 기준을 통과한다.
