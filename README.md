...
0.39.67부터 `ZDP-MONEY-004`는 `zdp-money-platform`의 `contracts/money-db-schema.yaml` payment webhook processing/outbox 계약을 검사한다. provider webhook event와 processing history append-only, raw provider payload 저장 금지, payment outbox 필수 필드, delivery status, bounded delivery attempt, aggregate/event/idempotency scope가 사라지면 실패한다.
