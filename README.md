...
- `zdp-platform-runtime` 저장소 루트의 health/readiness, smoke target, deployment template, rollback 계약이 배포 전 gate로 유지되는지 검사한다.
