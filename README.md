...
- `zdp-edge-workers` 저장소 루트의 request boundary, webhook ingress, queue envelope, analytics ingress 계약이 Worker 구현 전 gate로 유지되는지 검사한다.
...
- `zdp-data-platform` 저장소 루트의 analytics ingest, ClickHouse storage, deletion/anonymization 계약과 checker skeleton이 GA4 대체 구현 전 gate로 유지되는지 검사한다.
