...
- `zdp-libs-ts` 저장소 루트의 API contract source/package/schema/env/event/error/i18n 계약과 checker skeleton이 공통 TypeScript 패키지 구현 전 gate로 유지되는지 검사한다.
- `zdp-client-sdks` 저장소 루트의 SDK generation source handoff, sdk surface, auth helper, upload client 계약과 checker skeleton이 SDK 구현 전 gate로 유지되는지 검사한다.
