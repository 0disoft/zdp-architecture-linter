...
- `zdp-core-platform` 저장소 루트의 core boundary, command envelope, audit event, consent record, auth/session runtime handoff, identity session store 계약 파일이 구현 전 gate로 유지되는지 검사한다.
